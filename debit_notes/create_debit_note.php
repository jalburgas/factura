...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Verificar si ya existe una nota de débito con los mismos detalles
    $check_sql = "SELECT id FROM debit_notes WHERE invoice_id = ? AND amount = ? AND reason = ?";
    ...
    if ($check_result->num_rows > 0) {
        echo "Error: Ya existe una nota de débito con los mismos detalles.";
    } else {
        // Insertar la nota de débito
        $sql = "INSERT INTO debit_notes (invoice_id, cliente_id, amount, reason, created_at) VALUES (?, ?, ?, ?, NOW())";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('iids', $invoice_id, $cliente_id, $amount, $reason);

        if ($stmt->execute()) {
            $debit_note_id = $stmt->insert_id;
            header("Location: create_debit_note.php");
            exit();
        } else {
            echo "Error al crear la nota de débito: " . $conn->error;
        }
    }
}

// Consulta para obtener las notas de débito junto con el cliente_id de las facturas
$sql = "SELECT dn.id, dn.invoice_id, dn.cliente_id, dn.amount, dn.reason, dn.created_at
        FROM debit_notes dn
        JOIN facturas f ON dn.invoice_id = f.id";
$result = $conn->query($sql);
$debit_notes = $result->fetch_all(MYSQLI_ASSOC);
?>
...
<div class="container mt-5">
    <h2>Crear Nota de Débito</h2>
    <form method="POST" action="create_debit_note.php">
        ...
        <button type="submit" class="btn btn-primary">Crear Nota de Débito</button>
    </form>

    <h2 class="mt-5">Notas de Débito Creadas</h2>
    <table id="debitNotesTable" class="display">
        <thead>
            <tr>
                <th>ID</th>
        ...
        <tbody>
            <?php foreach ($debit_notes as $row): ?>
                <tr>
                    ...
                    <td>
                        <a href="view_debit_note.php?id=<?php echo $row['id']; ?>" class="btn btn-primary">Ver e Imprimir</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
